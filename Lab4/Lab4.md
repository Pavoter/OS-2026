...
1. 比较朴素实现、内积形式（A·B^T）与外积形式（A·B）在数据复用与缓存友好性上的差异，并结合块划分/循环展开等策略解释性能差距；
2. 说明优化过程中采用的指令级或循环级手段（例如 4 元展开、对齐访问、预取/缓存命中优化），以及这些手段如何减少访存与流水线停顿；
3. 针对尾部处理、对齐限制与不同矩阵规模下的瓶颈给出后续可行的优化方向，如进一步的块大小调优或向量化实现。
